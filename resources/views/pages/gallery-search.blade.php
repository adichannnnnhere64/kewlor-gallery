...
new class extends Component {
    ...
    public function deleteLiveEvent(int $id)
    {
        ...
        return redirect()->route('gallery-search');
    }
    ...
    public function liveEvents()
    {
        $bargo = LiveEventGallery::with('media')
            ...
            ->when($this->search, function ($query) {
                $query->where('name', 'like', '%' . $this->search . '%')->orWhereHas('categories', function ($query) {
                    ...
                    <div class="grid w-full lg:grid-cols-5 sm:grid-cols-2 gap-2 mt-8 max-w-6xl ">
                        @foreach ($this->liveEvents as $key => $liveEvent)
                            <div class="relative group"
                                wire:key="mcard-{{ $liveEvent->id }}-{{ $this->sortBy }}-{{ $this->categoryFilter }}-{{ now()->timestamp }}">
                                <div
                                ...
                                <x-ui.card :sortBy="$sortBy" :categoryFilter="$categoryFilter" :model="$liveEvent" :categories="$liveEvent->categories->pluck('name', 'id')"
                                    :currentVote="$liveEvent->current_vote" :likesCount="$liveEvent->likes_count" :dislikesCount="$liveEvent->dislikes_count" :id="$liveEvent->id"
                                    :liveEventId="$liveEvent->id" :showComment="true" :title="$liveEvent->name" :description="$liveEvent->created_at->format('Y-m-d')"
                                    :image="$liveEvent
                                        ->getMedia('default')
                                        ->sortBy('order_column')
                                        ->first()
