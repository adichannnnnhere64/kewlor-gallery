<div x-data="{ handle: (item, position) => $wire.updateOrder(item, position) }" class="mb-5 overflow-x-auto bg-white rounded-lg shadow overflow-y-auto relative">
    <table class="border-collapse table-auto w-full whitespace-no-wrap bg-white table-striped relative">
        ...
        <tbody x-sort="handle" x-on:sorted="$wire.updateOrder($event)">
                ...
                <tr x-data="{
                    ...
                    @foreach ($values as $value)
                        <td x-show="!showConfirm" class="border-t border-gray-200">
                            ...
                            @if ($value['type'] === 'image')
                                <img src="{{ $item->{$valueItem[0]} }}" alt="{{ $item->{$valueItem[0]} }}"
                                    class=" rounded-full w-16 h-16 my-2 object-cover object-center" />
                            @endif
                                    ...
                                    @if ($actions->contains('edit'))
                                        @if (!empty($editRoute) && !empty($editId))
                                            <a class="transition duration-500 ease-in-out underline underline-indigo-200 text-indigo-500 mr-2"
                                                href="{{ route($editRoute, ['id' => $item[$editId]]) }}">Edit</a>
                                        @else
